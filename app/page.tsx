...
export default function Home() {
  ...
  const [deploymentSteps, setDeploymentSteps] = useState<DeploymentStep[]>([
    ...
    { message: "Requesting testnet ETH from faucet...", status: "pending" },
    { message: "Deploying contract...", status: "pending" },
    { message: "Waiting for transaction confirmation...", status: "pending" },
  ]);
  ...
  const deployContract = async (type: "ERC721" | "ERC1155") => {
    ...
    try {
      ...
      // Step 1 Complete
      setDeploymentSteps((current) =>
        current.map((step, index) => (index === 0 ? { ...step, status: "complete" } : step))
      );
  
      // Step 2: Requesting ETH from faucet
      setDeploymentSteps((current) =>
        current.map((step, index) => (index === 1 ? { ...step, status: "loading" } : step))
      );
      await new Promise(resolve => setTimeout(resolve, 1000)); // Simulate faucet time
  
      // Step 2 Complete
      setDeploymentSteps((current) =>
        current.map((step, index) => (index === 1 ? { ...step, status: "complete" } : step))
      );
  
      // Step 3: Deploying contract
      setDeploymentSteps((current) =>
        current.map((step, index) => (index === 2 ? { ...step, status: "loading" } : step))
      );
      await new Promise(resolve => setTimeout(resolve, 1000)); // Simulate deployment time
  
      // Step 3 Complete
      setDeploymentSteps((current) =>
        current.map((step, index) => (index === 2 ? { ...step, status: "complete" } : step))
      );
  
      // Step 4: Waiting for confirmation
      setDeploymentSteps((current) =>
        current.map((step, index) => (index === 3 ? { ...step, status: "loading" } : step))
      );
      await new Promise(resolve => setTimeout(resolve, 1000)); // Simulate confirmation time
  
      // Step 4 Complete
      setDeploymentSteps((current) =>
        current.map((step, index) => (index === 3 ? { ...step, status: "complete" } : step))
      );
    ...
    <main className="w-full min-h-screen p-8 flex flex-col items-center bg-gray-100">
          ...
          {showAttributeInfo && (
            <div className="bg-gray-50 p-4 rounded-lg mt-2 text-gray-800">
              ...
              <p className="mb-2">
                ...
                properties that make each NFT unique. These can vary based on
                the token standard:
              </p>
              <ul className="list-disc list-inside mb-2">
                <li>
                  For <strong>ERC-721</strong>, attributes often represent
                  traits such as visual features or status values.
                </li>
                <li>
      ...
      {deploying && (
        <div className="w-full max-w-md bg-blue-100 p-6 mt-6 rounded-lg shadow-md">
          <p className="text-blue-800">
            Attempting to deploy contract. This may take a few minutes as we
      ...
      {deployedAddress && (
        <div className="w-full max-w-md bg-green-100 p-6 mt-6 rounded-lg shadow-md">
          ...
          <p className="text-green-700">Wallet Balance: {walletBalance} ETH</p>
          <a
